# Quest City Web — staging backup/restore tooling image (Tranche E design
# report §21-23; mission §17). Needs both Node (to run
...
# the monorepo uses (tools/package.json), rather than pulling in the whole
# pnpm workspace (@quest-city-web/* packages, etc.) that this single-purpose
# image has no other use for.
RUN npm install --no-save pg@8.13.1
COPY tools/backup-staging-db.mjs tools/restore-staging-db.mjs tools/staging-backup-target-adapter.mjs tools/staging-healthcheck.mjs ./
