...
// Tranche 2, `07_26 v1.0` §16). Mirrors `tools/seed-tranche1-content-bundle.ts`
// exactly, for the third real content bundle this platform now has:
// `QUICK_QUESTION_SET`. No public write endpoint exists for `content_bundle`
// (AGENTS.md convention) — this script IS the controlled process.
//
// The seeded row uses a FIXED `id` (not the column default
// `gen_random_uuid()`) for the same reason as WEB-M4/Tranche 1's scripts:
...
//
// Usage:
//   pnpm --filter @quest-city-web/tools run seed:tranche2-content-bundle
...
const SEED_ACTOR_ID = "tools/seed-tranche2-content-bundle.ts@1.0.0";
...
function assertGitRootMatches(): void {
  try {
    ...
    // git not available or not a repo — best-effort only, same as seed-tranche1-content-bundle.ts.
  }
}
...
async function main(): Promise<void> {
  ...
  if (!validation.ok) {
    throw new Error(`WEB_TRANCHE2_QUICK_QUESTION_SET_BUNDLE_MANIFEST failed validation: ${validation.errors.join("; ")}`);
  }
  ...
  try {
    ...
    if (existing.rows.length > 0) {
      throw new Error(`Content bundle with public_id '${WEB_TRANCHE2_CONTENT_BUNDLE_PUBLIC_ID}' already exists — refusing to overwrite.`);
    }
    ...
    console.log(
      `Created content_bundle '${WEB_TRANCHE2_CONTENT_BUNDLE_PUBLIC_ID}' (id ${WEB_TRANCHE2_MAT_M06_CONTENT_BUNDLE_ID}, ` +
