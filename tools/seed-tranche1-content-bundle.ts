...
// Tranche 1, `07_26 v1.0` §14). Mirrors `tools/seed-content-bundle.ts`
// (WEB-M4) exactly, for the second real content bundle this platform now
// has: `GUIDED_PRACTICE` + `REFLECTION_AND_RESULT`. No public write
// endpoint exists for `content_bundle` (AGENTS.md convention) — this
// script IS the controlled process.
//
// The seeded row uses a FIXED `id` (not the column default
// `gen_random_uuid()`) for the same reason as WEB-M4's script:
...
//
// Usage:
//   pnpm --filter @quest-city-web/tools run seed:tranche1-content-bundle
...
const SEED_ACTOR_ID = "tools/seed-tranche1-content-bundle.ts@1.0.0";
...
async function main(): Promise<void> {
  ...
  if (!validation.ok) {
    throw new Error(`WEB_TRANCHE1_GUIDED_PRACTICE_BUNDLE_MANIFEST failed validation: ${validation.errors.join("; ")}`);
  }
  ...
  try {
    ...
    if (existing.rows.length > 0) {
      throw new Error(`Content bundle with public_id '${WEB_TRANCHE1_CONTENT_BUNDLE_PUBLIC_ID}' already exists — refusing to overwrite.`);
    }
    ...
    console.log(
      `Created content_bundle '${WEB_TRANCHE1_CONTENT_BUNDLE_PUBLIC_ID}' (id ${WEB_TRANCHE1_MAT_M06_CONTENT_BUNDLE_ID}, ` +
