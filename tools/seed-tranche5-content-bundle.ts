...
// Tranche 5, `07_26 v1.1` §13/§17). Mirrors `tools/seed-tranche4-content-bundle.ts`
// exactly, for the sixth real content bundle this platform now has:
// `INTRO_HOOK`. No public write endpoint exists for `content_bundle`
...
//
// Usage:
//   pnpm --filter @quest-city-web/tools run seed:tranche5-content-bundle
...
const SEED_ACTOR_ID = "tools/seed-tranche5-content-bundle.ts@1.0.0";
...
async function main(): Promise<void> {
  ...
  if (!validation.ok) {
    throw new Error(`WEB_TRANCHE5_INTRO_HOOK_BUNDLE_MANIFEST failed validation: ${validation.errors.join("; ")}`);
  }
  ...
  try {
    ...
    if (existing.rows.length > 0) {
      throw new Error(`Content bundle with public_id '${WEB_TRANCHE5_CONTENT_BUNDLE_PUBLIC_ID}' already exists — refusing to overwrite.`);
    }
    ...
    console.log(
      `Created content_bundle '${WEB_TRANCHE5_CONTENT_BUNDLE_PUBLIC_ID}' (id ${WEB_TRANCHE5_MAT_M06_CONTENT_BUNDLE_ID}, ` +
