...
// Tranche 3, `07_26 v1.0` §5/§13). Mirrors `tools/seed-tranche2-content-bundle.ts`
// exactly, for the fourth real content bundle this platform now has:
// `PREREQUISITE_CHECK` + `MICRO_LESSON`. No public write endpoint exists for
...
//
// Usage:
//   pnpm --filter @quest-city-web/tools run seed:tranche3-content-bundle
...
const SEED_ACTOR_ID = "tools/seed-tranche3-content-bundle.ts@1.0.0";
...
async function main(): Promise<void> {
  ...
  if (!validation.ok) {
    throw new Error(`WEB_TRANCHE3_PREREQUISITE_CHECK_BUNDLE_MANIFEST failed validation: ${validation.errors.join("; ")}`);
  }
  ...
  try {
    ...
    if (existing.rows.length > 0) {
      throw new Error(`Content bundle with public_id '${WEB_TRANCHE3_CONTENT_BUNDLE_PUBLIC_ID}' already exists — refusing to overwrite.`);
    }
    ...
    console.log(
      `Created content_bundle '${WEB_TRANCHE3_CONTENT_BUNDLE_PUBLIC_ID}' (id ${WEB_TRANCHE3_MAT_M06_CONTENT_BUNDLE_ID}, ` +
